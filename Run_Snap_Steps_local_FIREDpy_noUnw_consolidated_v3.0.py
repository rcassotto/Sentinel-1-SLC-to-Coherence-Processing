...
#from SNAP_preProcessing_functions import rasterio_composite_functions as rc
...
class insar_processing_snap():
    ...
    # 1.
    # exe paths
    ##### RYAN TO AMEND THIS TO LOCAL LOCATIONS
    base_snap_exe = "/home/rcassotto/esa-snap/bin/gpt"
#    base_snap_exe_no_aux = "/home/rcassotto/snap/bin/gpt_no_orbit"  # RKC mod specific to node 02 Jul 26, 2023
    base_snap_exe_no_aux = "/home/rcassotto/esa-snap/bin/gpt"
    snaphu_exe = "/usr/bin/snaphu"
    gdal_exe = "/usr/bin/gdal_translate"
#    gdal_exe = "/home/rcassotto/anaconda3/envs/FIREDpy/bin/gdal_translate"
...
#    Multilook_Param_Keys = ['num_rg_Looks'] # original 
    ...
    if lon_1 == None and lat_1 == None and lat_2 == None and lon_2 == None:
        ...
        #                       lat_2 + ', ' + lon_3 + ' ' + lat_1 + ', ' + lon_1 + ' ' + lat_1 + '))", subSamplingX, subSamplingY]
    ...
    def __init__(self, sys_argv_int, json_dir, output_pix_size, param_dict=all_Parameters):
   # def __init__(self, sys_argv_int, json_dir, output_pix_size, lon_1, lon_2, lat_1, lat_2, param_dict=all_Parameters):
      ...
      # self.base_path = sys_argv_path 
        self.base_path_name = self.base_path.name
        self.pair_path = self.base_path.parent
#        self.aux_poe = '/data2/Sentinel1_orbits/aux_poeorb'
#        self.res_orb = '/data2/Sentinel1_orbits/aux_resorb'
        # self.aux_poe = '/data/Work/Sentinel1_orbits/aux_poeorb'
#        self.res_orb = '/data/Work/Sentinel1_orbits/aux_resorb'        
...
#        self.data_path = self.base_path / 'Main/Data'  ## RKC commented out
#        self.pairs_path = self.base_path / 'Main/Pairs'
        self.data_path = json_dir ## Added 4/6/23 to try and remove /Main/xxxx hard coded structure
        self.pairs_path = json_dir + "/Pairs"
        print('Data path is: ', self.data_path)          
        self.DEM_path = json_dir + "/DEM" # RKC mod for removing Main/DEM hard code
...
def read_ImagePairsJSON_file(json_dir, json_filename):
    ## Read in Pairs_List.json file
  f = open(json_dir + json_filename)
...
#    return spatial_subset_keys, spatial_subset_vals, wkt_string, lon_1, lon_2, lat_1, lat_2
    return spatial_subset_keys, spatial_subset_vals, wkt_string
  
if __name__ == '__main__':
    
    ### Read input parameter file (e.g. FIREDpy_input_slc_proc.txt)
    ...
    json_dir = args['slc_file_loc'] # get location of SLC *.zip files from input.txt file
    output_pix_size = args['output_resolution_m']
    roi_specified = args['fire_roi_polygon']
    roi_polygon_path = args['fire_roi_path']
    sys_index_var = int(args['sys_index_var'])

    ### Sanity check 
    print('polygon filename: ', roi_specified, type(roi_specified), not roi_specified)   
    print("Data directory: ", json_dir)
    print("Output Pixel Size (m): ", output_pix_size, type(output_pix_size))
   ...
   # (spatial_subset_keys, spatial_subset_vals, wkt_string, lon_1, lon_2, lat_1, lat_2) = define_spatial_subset_parameters(roi_specified, roi_polygon_path)
    (spatial_subset_keys, spatial_subset_vals, wkt_string) = define_spatial_subset_parameters(roi_specified, roi_polygon_path)

    
    print("keys:", spatial_subset_keys)
    print("vals:", spatial_subset_vals)


   ### First iteration - to generate Pairs_list.json file and Meta_Data and kml_frame directories 
  #  sys_index_var = int(sys.argv[1])    
    print("sys index var: " , sys_index_var, type(sys_index_var))
    run_insar_processing_object = insar_processing_snap(sys_index_var, json_dir, output_pix_size)  # RKC mod to specify user defined Data path
    #run_insar_processing_object = insar_processing_snap(sys_index_var, json_dir, wkt_string, lon_1, lon_2, lat_1, lat_2)  # RKC 20231026: mod to pass wkt_string from geojson polygon
            

    if sys_index_var == 0:
        print("Initializing.....Processing first pair")
        run_insar_processing_object = insar_processing_snap(sys_index_var, json_dir, output_pix_size)  # RKC mod to specify user defined Data path
 #       run_insar_processing_object = insar_processing_snap(sys_index_var, json_dir, output_pix_size, wkt_string, lon_1, lon_2, lat_1, lat_2) # RKC 20231026: mod to pass wkt_string from geojson polygon
  ...
  ## Read in json file to get list of Pairs  
    json_data = read_ImagePairsJSON_file(json_dir, "/Pairs_List.json")
    nPairs = len(json_data)
    
    ## Iterate over the remaining pairs to generate coherence images for each
   # for inPair in range(1,nPairs): # original
...
#        run_insar_processing_object = insar_processing_snap(inPair, json_dir)  # RKC mod
        run_insar_processing_object = insar_processing_snap(inPair, json_dir, output_pix_size)  # RKC mod
       # run_insar_processing_object = insar_processing_snap(inPair, json_dir, output_pix_size, wkt_string)  # RKC 20231026: mod to pass wkt_string from geojson polygon
        run_insar_processing_object.run_insar_processing()
